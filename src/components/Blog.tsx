...
export const Blog = ({ posts }: { posts: Post[] }) => {
  return (
    <section id="blog" className="py-24 px-8 md:px-24 bg-obsidian border-b border-slate-gray/30">
      <div className="mb-16">
      ...
      <div className="space-y-0 divide-y divide-slate-gray/20">
        {posts.length > 0 ? (
          posts.map((post, index) => (
            ...
            >
              <motion.div
                ...
                viewport={{ once: true }}
                className="py-8 first:pt-0 group-hover:bg-white/[0.02] transition-colors"
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-pure-white/40 text-sm leading-relaxed max-w-3xl">
                      {post.excerpt}
                    </p>
                  </div>
          ...
          <p className="py-8 text-slate-gray font-mono italic">[EMPTY] No logs found in directory.</p>
        )}
      </div>
      ...
      {posts.length > 0 && (
        <motion.button 
          whileHover={{ x: 5 }}
          className="mt-12 flex items-center gap-2 text-xs font-mono uppercase tracking-[0.2em] text-slate-gray hover:text-pure-white transition-colors"
        >
